# -*- coding: utf-8 -*-
"""
كشف التكرارات - خطوة بخطوة
"""

from core.data_loader import DataLoader
from core.duplicate_analyzer import DuplicateAnalyzer

print("="*80)
print("🔍 كشف التكرارات - خطوة بخطوة")
print("="*80)

# ============================================================================
# الخطوة 1: تحميل البيانات
# ============================================================================
print("\n📥 الخطوة 1: تحميل البيانات...")
print("-" * 80)

loader = DataLoader('العجوري 11-4.csv')
df = loader.load()

print(f"✅ تم تحميل {len(df):,} صف و {len(df.columns)} عمود")
print(f"\n📋 أسماء الأعمدة:")
for i, col in enumerate(df.columns, 1):
    print(f"   {i:2d}. {col}")

input("\n⏸️  اضغط Enter للمتابعة إلى الخطوة التالية...")

# ============================================================================
# الخطوة 2: تنظيف البيانات
# ============================================================================
print("\n" + "="*80)
print("🧹 الخطوة 2: تنظيف البيانات...")
print("-" * 80)

loader.auto_clean()
df = loader.get_data()

print(f"✅ بعد التنظيف: {len(df):,} صف و {len(df.columns)} عمود")
print(f"\n📋 الأعمدة بعد التنظيف:")
for i, col in enumerate(df.columns, 1):
    null_pct = df[col].isnull().sum() / len(df) * 100
    print(f"   {i:2d}. {col:30s} - قيم فارغة: {null_pct:.1f}%")

input("\n⏸️  اضغط Enter للمتابعة إلى الخطوة التالية...")

# ============================================================================
# الخطوة 3: عرض عينة من البيانات
# ============================================================================
print("\n" + "="*80)
print("👀 الخطوة 3: عرض عينة من البيانات...")
print("-" * 80)

print("\n📊 أول 5 صفوف:")
print(df.head(5).to_string())

input("\n⏸️  اضغط Enter للمتابعة إلى الخطوة التالية...")

# ============================================================================
# الخطوة 4: تحديد الأعمدة للتحليل
# ============================================================================
print("\n" + "="*80)
print("🎯 الخطوة 4: تحديد الأعمدة للتحليل...")
print("-" * 80)

# اختيار أعمدة مناسبة للتحليل
print("\n📋 اختيار الأعمدة:")

# عرض الأعمدة المتاحة
print("\nالأعمدة المتاحة:")
for i, col in enumerate(df.columns, 1):
    print(f"   {i:2d}. {col}")

# تحديد الأعمدة تلقائياً
entity_col = 'OwnerName'  # عمود الجهة
amount_col = 'EntryTypeNumber'  # عمود المبلغ (رقمي)
event_col = 'Race'  # عمود الحدث

print(f"\n✅ الأعمدة المختارة:")
print(f"   • عمود الجهة: {entity_col}")
print(f"   • عمود المبلغ: {amount_col}")
print(f"   • عمود الحدث: {event_col}")

input("\n⏸️  اضغط Enter للمتابعة إلى الخطوة التالية...")

# ============================================================================
# الخطوة 5: البحث عن التكرارات
# ============================================================================
print("\n" + "="*80)
print("🔍 الخطوة 5: البحث عن التكرارات...")
print("-" * 80)

analyzer = DuplicateAnalyzer(df)

print("\n⏳ جاري البحث...")
duplicates = analyzer.find_payment_duplicates(
    entity_col=entity_col,
    amount_col=amount_col,
    event_col=event_col
)

print(f"\n✅ اكتمل البحث!")
print(f"\n📊 النتائج:")
print(f"   • إجمالي السجلات: {len(df):,}")
print(f"   • عدد التكرارات: {len(duplicates):,}")
print(f"   • نسبة التكرار: {len(duplicates)/len(df)*100:.2f}%")

if len(duplicates) > 0:
    print(f"   • عدد المجموعات المكررة: {duplicates['duplicate_group'].nunique()}")
    
    input("\n⏸️  اضغط Enter لعرض التكرارات...")
    
    # ============================================================================
    # الخطوة 6: عرض التكرارات
    # ============================================================================
    print("\n" + "="*80)
    print("📋 الخطوة 6: عرض التكرارات...")
    print("-" * 80)
    
    # عرض أول 20 تكرار
    print("\n📄 أول 20 تكرار:")
    display_cols = [entity_col, amount_col, event_col, 'duplicate_group', 'duplicate_count']
    print(duplicates[display_cols].head(20).to_string())
    
    # إحصائيات المجموعات
    print("\n📊 إحصائيات المجموعات:")
    group_stats = duplicates.groupby('duplicate_group').agg({
        entity_col: 'first',
        amount_col: 'first',
        'duplicate_count': 'first'
    }).sort_values('duplicate_count', ascending=False)
    
    print("\n🔝 أكثر 10 مجموعات تكراراً:")
    print(group_stats.head(10).to_string())
    
    input("\n⏸️  اضغط Enter للمتابعة إلى التصدير...")
    
    # ============================================================================
    # الخطوة 7: تصدير النتائج
    # ============================================================================
    print("\n" + "="*80)
    print("💾 الخطوة 7: تصدير النتائج...")
    print("-" * 80)
    
    output_file = 'outputs/duplicates_detailed_report.xlsx'
    analyzer.export_duplicates(output_file)
    
    print(f"\n✅ تم تصدير التقرير بنجاح!")
    print(f"📁 الموقع: {output_file}")
    
else:
    print("\n✅ لم يتم العثور على تكرارات!")

# ============================================================================
# الخطوة 8: البحث عن تطابق ضبابي (اختياري)
# ============================================================================
print("\n" + "="*80)
print("🔍 الخطوة 8: البحث عن تطابق ضبابي (أسماء متشابهة)...")
print("-" * 80)

print("\n⏳ جاري البحث عن أسماء متشابهة...")
fuzzy_matches = analyzer.find_fuzzy_duplicates(
    name_col=entity_col,
    threshold=0.90  # 90% تشابه أو أكثر
)

if len(fuzzy_matches) > 0:
    print(f"\n✅ تم العثور على {len(fuzzy_matches)} تطابق ضبابي")
    print("\n📋 أمثلة على الأسماء المتشابهة:")
    for i, row in fuzzy_matches.head(10).iterrows():
        print(f"\n   {i+1}. التطابق:")
        print(f"      • الاسم 1: {row['original_name1']}")
        print(f"      • الاسم 2: {row['original_name2']}")
        print(f"      • نسبة التشابه: {row['similarity']*100:.1f}%")
else:
    print("\n✅ لم يتم العثور على تطابقات ضبابية")

# ============================================================================
# النهاية
# ============================================================================
print("\n" + "="*80)
print("✅ اكتمل التحليل بنجاح!")
print("="*80)
print("\n📌 الملخص:")
print(f"   • تم تحليل: {len(df):,} سجل")
print(f"   • تكرارات تامة: {len(duplicates):,}")
if len(fuzzy_matches) > 0:
    print(f"   • تطابقات ضبابية: {len(fuzzy_matches)}")
print(f"\n📁 التقارير محفوظة في مجلد: outputs/")
print("\n🌐 للاستخدام التفاعلي: http://localhost:8501")
print("\n")
